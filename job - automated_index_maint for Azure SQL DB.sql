...
Note: Enable @CompressWhenPossible to start enabling data_compression = PAGE everywhere. Not recommended without testing.
Note: Outside of Azure SQL DB, see toolbox\job - automated_index_maint for Azure SQL DB.sql
...
#TODO 
1. Create the logging table inside each Azure SQL DB using table create script at bottom. 
	If desired, change the schema from dbo to audit or admin or other schema.
	
2. Create the sproc dbo.Sparkhound_automated_index_maint in each database it needs to maintain.
	Note: if adding a stored procedure to the database is not allowed, use the sproc-less option in toolbox\automated_index_maint.sql.

3. Be aware that all Azure SQL Databases are at UTC +00:00 time zones. You must pass in start/end hour windows in a desired time zone for each database, regardless of the location of the database.
	Use only values from sys.time_zone_info, provide a value from the name column here. Examples:
		...@Target_Time_Zone nvarchar(256) = 'Pacific Standard Time', ...
		...@Target_Time_Zone nvarchar(256) = 'US Mountain Standard Time', ...
		...@Target_Time_Zone nvarchar(256) = 'Central Standard Time', ...
		...@Target_Time_Zone nvarchar(256) = 'Eastern Standard Time', ...
		...@Target_Time_Zone nvarchar(256) = 'Central Europe Standard Time', ...

4. Create a SQL Agent job to run the sproc in each database. The job step command should reflect the hour of day in the target time zone, not UTC, for example:
		Exec dbo.Sparkhound_automated_index_maint 2,4,'Central Standard Time' --run between 2AM-4AM Central Standard Time, no compression, no Testmode
		or
		Exec dbo.Sparkhound_automated_index_maint @StartWindow = 2, @EndWindow =4, @Target_Time_Zone = 'Central Standard Time' --run between 2AM-4AM Central Standard Time, no Testmode, no compression
		or
		Exec dbo.Sparkhound_automated_index_maint @StartWindow = 2, @EndWindow =4, @testMode = 1 --run now with Testmode ON (ignores Windows), no compression
		or
		Exec dbo.Sparkhound_automated_index_maint @StartWindow = 2, @EndWindow =4, @Target_Time_Zone = 'Central Standard Time', @testMode = 0, @CompressWhenPossible = 1--run between 2AM-4AM Central Standard Time, add compression to all indexes found
		or
		Exec dbo.Sparkhound_automated_index_maint 2,4,'Central Standard Time',0,1 --run between 2AM-4AM Central Standard Time, add compression to all indexes found
	
4. Schedule the job to start inside the start window. Ex: @StartWindow = 2, then start the SQL Agent job between 2AM - 2:59AM.

*/
Exec dbo.Sparkhound_automated_index_maint 15,16,'Central Standard Time'
select * from dbo.indexmaintlog
GO
ALTER PROCEDURE dbo.Sparkhound_automated_index_maint (@StartWindow tinyint, @EndWindow tinyint, @Target_Time_Zone nvarchar(256) = 'Central Standard Time', @TestMode bit = 0, @CompressWhenPossible bit = 0)
AS
BEGIN

	IF NOT EXISTS (select * FROM sys.time_zone_info tzi where tzi.name = @Target_Time_Zone)
			THROW 51000, '--Invalid Desired Time Zone specified. Refer to sys.time_zone_info.', 1;  --stop if this database isn't online or is read_only. It may have already failed before this point anyway.

	--Convert Start/End windows from Desired Time Zone to UTC
	SELECT	@StartWindow  = CASE WHEN @StartWindow - cast(left(tzi.current_utc_offset, 3) as int) > 23 THEN @StartWindow - cast(left(tzi.current_utc_offset, 3) as int) - 24 ELSE @StartWindow - cast(left(tzi.current_utc_offset, 3) as int) END
		,	@EndWindow  = CASE WHEN @EndWindow - cast(left(tzi.current_utc_offset, 3) as int) > 23 THEN @EndWindow - cast(left(tzi.current_utc_offset, 3) as int) - 24 ELSE @EndWindow - cast(left(tzi.current_utc_offset, 3) as int) END
	FROM sys.time_zone_info tzi where tzi.name = @Target_Time_Zone
	...
	SELECT @maxlogid = ISNULL(MAX(id),0) From dbo.IndexMaintLog
	...
	BEGIN TRY 
				...
				BEGIN
					BEGIN TRY 
							...
							BEGIN --Begin Time frame
									...
									IF @Command <> ''
									BEGIN
										INSERT INTO dbo.IndexMaintLog (CurrentDatabase, Command, ObjectName, BeginTimeStamp, StartWindow, EndWindow, TestMode)
		...
		BEGIN CATCH
			...
			BEGIN
				...
				INSERT INTO dbo.IndexMaintLog (CurrentDatabase, ErrorMessage , BeginTimeStamp, TestMode, ObjectName, Command)
		...
		IF @TestMode = 0 AND EXISTS (SELECT 1 from dbo.IndexMaintLog where id > @maxlogid and ErrorMessage is not null)
			BEGIN
				DECLARE @errorcount int, @errormessage varchar(1000);
				SELECT @errorcount = count(ErrorMessage) from dbo.IndexMaintLog where id > @maxlogid and ErrorMessage is not null
				SELECT @errormessage = cast(@errorcount as varchar(100)) + ' error(s) detected while performing index maintainence. Review the findings in table: SELECT * FROM dbo.IndexMaintLog where id > '+ cast(@maxlogid as varchar(100));
...
/*
DROP TABLE IF EXISTS dbo.IndexMaintLog;
CREATE TABLE dbo.IndexMaintLog
