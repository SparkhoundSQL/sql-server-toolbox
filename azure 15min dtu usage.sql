...
,	'Average Log Write Utilization In %'	=	AVG(avg_log_write_percent)		   
,	'Maximum Log Write Utilization In %'	=	MAX(avg_log_write_percent)		   
,	'Average Worker Requests In %'					=	AVG(max_worker_percent)	
,	'Maximum Worker Requests In %'					=	MAX(max_worker_percent)	
,	'Average Sessions In %'					=	AVG(max_session_percent)	
,	'Maximum Sessions In %'					=	MAX(max_session_percent)	
--select *
from 
master.sys.resource_stats as rs  --past 14 days only
group by Database_Name, sku, dtu_limit, datetimefromparts(year(rs.end_time), month(rs.end_time), day(rs.end_time), datepart(hh,rs.end_time), datepart(minute, rs.end_time),0,0)
