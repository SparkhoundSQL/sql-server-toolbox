...
1. Check the name of the DBALogging/DBAAdmin/DBAHound database local to this SQL instance and replace if necessary.
	Note: In Azure SQL DB, see toolbox\job - automated_index_maint for Azure SQL DB.sql
...
3. Create a SQL Agent job to run the sproc in each database. The job step command should be, for example:
		Exec dbo.Sparkhound_automated_index_maint 2,4 --run between 2AM-4AM, no compression, no Testmode
		or
		Exec dbo.Sparkhound_automated_index_maint @StartWindow = 2, @EndWindow =4 --run between 2AM-4AM, no Testmode, no compression
		or
		...
		or
		Exec dbo.Sparkhound_automated_index_maint @StartWindow = 2, @EndWindow =4, @testMode = 0, @CompressWhenPossible = 1--run between 2AM-4AM, add compression to all indexes found
		or
		Exec dbo.Sparkhound_automated_index_maint 2,4,0,1 --run between 2AM-4AM, add compression to all indexes found
	
4. Schedule the job to start inside the start window. Ex: @StartWindow = 2, then start the SQL Agent job between 2AM - 2:59AM.

NOTE: If upgrading this script from an older version, look at the bottom to see if the index_maint_log Logging table needs data type changes. 
		We changed the timestamp field data types to datetimeoffset a couple years ago and may need to update the logging table. 
		
*/
USE AdventureWorks --TODO #2: change this
go

CREATE PROCEDURE dbo.Sparkhound_automated_index_maint (@StartWindow tinyint, @EndWindow tinyint, @TestMode bit = 0, @CompressWhenPossible bit = 0)
AS
BEGIN
