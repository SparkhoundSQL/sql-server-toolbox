...
--Performs diff backups on up all user databases
...
BEGIN  
	SET @BackupfileName = @BackupLoc + @DB_name + '_diff_' + 
	...
	BEGIN TRY
       BACKUP DATABASE @DB_name TO URL = @BackupfileName  
		...
		select @errormessage = N'Verify failed. Backup information for differential backup for database  '+@DB_name+' not found.'
